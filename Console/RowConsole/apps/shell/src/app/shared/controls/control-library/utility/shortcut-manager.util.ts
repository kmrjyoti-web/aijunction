...
export class ShortcutManager {
  private static listeners: { combo: string, callback: ShortcutCallback, stopPropagation: boolean }[] = [];
   ...
   */
  static register(combo: string, callback: ShortcutCallback, stopPropagation: boolean = true) {
    this.init();
    this.listeners.push({ combo: combo.toLowerCase(), callback, stopPropagation });
  }
  ...
  private static handleKey(e: KeyboardEvent) {
    ...
    // Map some special keys if needed, but usually e.key.toLowerCase() works well
    ...
    if (match) {
      ...
      const isSpecialKey = ['escape', 'enter', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9', 'f10', 'f11', 'f12', 'delete'].includes(e.key.toLowerCase());
      ...
      console.log(`[ShortcutManager] Triggered: ${currentCombo}`);
      match.callback(e);
    }
  }
}
